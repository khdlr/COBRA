...
def substep(data, size):
    ...
    O_m, O_y, O_x, O_py, O_px = jnp.split(data, 5)
    O_d = jnp.where(O_py == -1, jnp.inf, dist(O_y, O_x, O_py, O_px))
    ...
    B_m, B_y, B_x, _, _ = jnp.split(jax.vmap(get_neighbor)(neighbors)[:, 0], 5)
    B_d = jnp.take_along_axis(N_d, best_neighbor, axis=0)

    data = jnp.where(
        (B_m != O_m) & (B_d < O_d), jnp.concatenate([O_m, O_y, O_x, B_y, B_x]), data
    )
    ...
    best_pointer = argminwhere(dist2pointer, (O_m == N_m) & (N_py > -1), axis=0)[
        jnp.newaxis
    ]
    get_pointer = partial(jnp.take_along_axis, indices=best_pointer, axis=0)
    P_m, _, _, P_y, P_x = jnp.split(jax.vmap(get_pointer)(neighbors)[:, 0], 5)
    P_d = jnp.take_along_axis(dist2pointer, best_pointer, axis=0)

    data = jnp.where(
        (P_m == O_m) & (P_y != -1) & (P_d < O_d),
        jnp.concatenate([O_m, O_y, O_x, P_y, P_x]),
        data,
    )
    return data, None
