...
class CALFINXception(hk.Module):
    ...
    def __call__(self, x, is_training=False):
        ...
        # Backbone
        x, skip1 = CALFINXceptionBlock([128, 128, 128], stride=2, return_skip=True)(
            x, is_training
        )
        x, skip2 = CALFINXceptionBlock([256, 256, 256], stride=2, return_skip=True)(
            x, is_training
        )
        x, skip3 = CALFINXceptionBlock([768, 768, 768], stride=2, return_skip=True)(
            x, is_training
        )
        for i in range(8):
            x = CALFINXceptionBlock([768, 768, 768], skip_type="sum", stride=1)(
                x, is_training
            )

        x = CALFINXceptionBlock([728, 1024, 1024], stride=2)(x, is_training)
        x = CALFINXceptionBlock([1536, 1536, 2048], stride=1, rate=(1, 2, 4))(
            x, is_training
        )
