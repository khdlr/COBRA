...
def calfin_loss(segmentation, edge, mask):
    ...
    edge = edge[..., 0]
    true_edge = hk.max_pool(mask, [5, 5], [1, 1], "SAME") != min_pool(
        mask, [5, 5], [1, 1], "SAME"
    )

    seg_loss = jnp.mean(_bce(segmentation, mask)) + jnp.mean(
        _iou_loss(segmentation, mask)
    )
    edge_loss = jnp.mean(_bce(edge, true_edge)) + jnp.mean(_iou_loss(edge, true_edge))
...
def hed_unet_loss(hed_unet_stack, mask):
    ...
    edge = hk.max_pool(mask, [3, 3], [1, 1], "SAME") != min_pool(
        mask, [3, 3], [1, 1], "SAME"
    )
...
def dance_loss(snake_steps, edge, mask):
    ...
    for i, (weight, snake) in enumerate(zip(weights, snake_steps[1:])):
        loss_terms[f"stage_{i}_loss"] = 10 * optax.huber_loss(
            snake, contour, delta=0.033
        )
...
def _balanced_bce(prediction, mask):
    ...
    loss = -jnp.mean((1 - beta) * logit1 * mask + beta * logit0 * (1 - mask))
